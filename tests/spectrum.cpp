...
TEST(Spectrum, new_with_floats) {
  auto s = new Spectrum(0.5, 0.6, 0.7);
...
TEST(Spectrum, new_from_other) {
  auto other = new Spectrum(0.5, 0.6, 0.7);
  auto s = new Spectrum(other);
  ASSERT_NEAR(0.5, s->r, EPS);
  ASSERT_NEAR(0.6, s->g, EPS);
  ASSERT_NEAR(0.7, s->b, EPS);
}
...
TEST(Spectrum, scale) {
  auto s = new Spectrum(0.4, 0.5, 0.6);
  s->scale(1.5);
...
TEST(Spectrum, add) {
  auto s = new Spectrum(0.4, 0.5, 0.6);
  auto t = new Spectrum(0.2, 0.25, 0.3);
  s->add(t);
  ASSERT_NEAR(0.6, s->r, EPS);
  ASSERT_NEAR(0.75, s->g, EPS);
  ASSERT_NEAR(0.9, s->b, EPS);
...
TEST(Spectrum, mult) {
  auto s = new Spectrum(0.4, 0.5, 0.6);
  auto t = new Spectrum(0.5, 0.1, 0.2);
  s->mult(t);
...
TEST(Spectrum, clamp) {
  auto s = new Spectrum(1.4, 2.5, 0.6);
  s->clamp();
