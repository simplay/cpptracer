...
HitRecord* Explosion::intersect(Ray* ray) const {
  ...
  for (size_t i = 0; i < 128; i++) {
    ...
    if (d < 0) {
      ...
      wIn->negate();
      wIn->normalize();
      return new HitRecord(t, new Vector3f(hitPos), norm, new Vector3f(), wIn,
                           material, this);
    }
  }
  return new HitRecord();
...
float Explosion::signed_distance(const Vector3f& p)
    const {  // this function defines the implicit surface we render
  Vector3f q(p);
  q.scale(3.4);
...
Vector3f Explosion::rotate(const Vector3f& v) const {
  Vector3f p(Vector3f(0.00, 0.80, 0.60).dot(&v),
